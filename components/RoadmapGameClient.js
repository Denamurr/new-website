...
const STAGE_POOLS = {
  0: [ // Discovery
    {
      ...
      choices: [
        { label: "Go with the CEO's favorite",            desc: "Why waste research when you already have a CEO?",               effects: { trust: 1 },              consequence: "Team ships the CEO's gut. At least the roadmap review will be short." },
        { label: "Run a quick validation sprint",         desc: "Spend a week confirming what you probably already know.",        effects: { quality: 1 },               consequence: "The research disagrees with the CEO. This is described as a research problem." },
        { label: "Pick the option with strongest signal", desc: "Use the user research before it expires.",                      effects: { scope: 1 },               consequence: "The data was right. Nobody sends data a thank-you note." },
        { label: "Scope a hybrid of all three",           desc: "Give everyone a little of what they want and none of what they need.", effects: { scope: -1 }, consequence: "Congratulations. You built nobody's favorite product." },
      ],
    },
    {
      ...
      choices: [
        { label: "Synthesize all three into one definition",   desc: "Create a definition vague enough that everyone nods.",                     effects: { scope: 1 }, consequence: "You made one definition that technically includes all three. Nobody is wrong. Nobody is aligned." },
        { label: "Schedule 1:1s with each before the meeting", desc: "Do the meeting before the meeting.",                                        effects: { trust: 1 },           consequence: "The stakeholders politely agree in public after disagreeing professionally in private." },
        { label: "Default to the CEO's view",                  desc: "Ultimately we should align with leadership's vision.",                      effects: { morale: -1 },          consequence: "Expedient. The other two stakeholders will remember." },
        { label: "Write a framing doc and circulate it",       desc: "Document the options and hope the argument happens in comments instead.",   effects: { quality: 1 },          consequence: "A written artifact. In 6 months this doc will either vindicate you or be used against you." },
      ],
    },
    {
      ...
      choices: [
        { label: "Run 10 more interviews",      desc: "Continue gathering evidence until someone tells you to stop.",          effects: { scope: -1 },            consequence: "You discover all three duplicate survey responses belonged to your CEO." },
        { label: "Proceed with what we have",   desc: "Good enough for a roadmap.",                                           effects: { quality: -1 },            consequence: "The team ships. Future-you inherits the consequences." },
        { label: "Run a 2-day research sprint", desc: "Let's gather just enough evidence to feel confident.",                 effects: { quality: 1 }, consequence: "Just enough rigor to survive the stakeholder meeting." },
        { label: "Outsource the research",      desc: "Let's get a consultant to tell us the same thing more confidently.",   effects: { trust: -1 },  consequence: "A 68-slide deck arrives. Slide 67 contains your original conclusion. Slide 68 says 'next steps.'" },
      ],
    },
    {
      ...
      choices: [
        { label: "Go back to users and validate",        desc: "Talk to the people who actually pay you.",          effects: { quality: 1 }, consequence: "After eight more interviews, the answer is 'it depends.' Research has once again delivered." },
        { label: "Trust the original research",          desc: "In surveys we trust.",                              effects: { trust: -1 },            consequence: "Survey respondents continue being your most enthusiastic customers." },
        { label: "Adjust pricing model and move on",     desc: "If you pivot fast enough, it's strategy.",          effects: { scope: 1 },  consequence: "Your pricing now contains the word 'Enterprise.' Revenue does not." },
        { label: "Flag the risk in Notion and continue", desc: "Future archaeologists will find your warning.",      effects: { quality: -1 },            consequence: "Three months later, someone discovers the note and reacts with a thumbs up." },
      ],
    },
    {
      ...
      choices: [
        { label: "Run it. Customer insight is good.", desc: "Customer conversations are research. Sort of.",    effects: { scope: -1 }, consequence: "One customer casually mentions a problem nobody on the team had considered. Six months of roadmap planning immediately enter the witness protection program." },
        { label: "Redirect to structured interviews", desc: "Reframe the chaos as a methodology.",             effects: { quality: 1 },            consequence: "The CEO wanted a vision session. The customers got interview questions. Everyone was mildly annoyed, but the notes were surprisingly useful." },
        { label: "Let the CEO run it.",               desc: "What could possibly go wrong?",                   effects: { trust: 1 },             consequence: "Ninety minutes later the customers know everything about the product and you've learned absolutely nothing about them." },
        { label: "Combine it with the sprint review", desc: "Two birds, one deeply confused meeting.",         effects: { morale: -1 }, consequence: "Customers watched bug fixes. Engineers watched customer feedback. Nobody knew which audience they were supposed to be performing for." },
      ],
    },
    {
      ...
      choices: [
        { label: "Build features for all of them",         desc: "Five edge cases enter the roadmap.",                          effects: { scope: -1 },    consequence: "The core experience leaves quietly through the back door." },
        { label: "Focus on the most common problem",       desc: "The squeakiest wheel receives the roadmap.",                  effects: { scope: 1 },   consequence: "You fixed the biggest issue. Users immediately begin discussing the second biggest issue." },
        { label: "Ignore interviews and trust the vision", desc: "The research was directional. Direction: forward.",           effects: { quality: -1 },   consequence: "Comfortable. The interviews are still in a folder labeled 'Research — DO NOT DELETE.'" },
        { label: "Segment the users",                      desc: "Maybe they're not actually the same customer.",               effects: { quality: 1 }, consequence: "You discover there are two user types. Marketing discovers there are seven." },
      ],
    },
    {
      ...
      choices: [
        { label: "Build exactly what they described", desc: "The whiteboard has spoken.",                              effects: { quality: -1 },           consequence: "The arrows survive every planning meeting. Nobody remembers what they originally pointed to." },
        { label: "Ask users first",                   desc: "Let's see if customers agree with the whiteboard.",       effects: { quality: 1 }, consequence: "The research created clarity. Several stakeholders found this inconvenient." },
        { label: "Suggest starting smaller",          desc: "Attempt to make the project survivable.",                 effects: { scope: 1 },  consequence: "The CEO did not enjoy this suggestion. You are no longer invited to whiteboard sessions." },
        { label: "Schedule a strategy workshop",      desc: "Surely another meeting will help.",                       effects: { morale: -1 }, consequence: "Four new arrows are added. Progress remains theoretical." },
      ],
    },
  ],
  ...
  1: [ // Strategy
  {
    ...
    choices: [
      { label: "Pretend it was always part of the plan",   desc: "History is written by whoever updates the roadmap.",      effects: { trust: 1 }, consequence: "Three people nod. One of them built the roadmap and knows you're lying." },
      { label: "Reply with a clear-headed tradeoff",       desc: "A risky amount of honesty.",                              effects: { trust: 1 },              consequence: "The CEO responds with 'Good points.' This is either very good or very bad." },
      { label: "Add it and cut something else",            desc: "Practice the lost art of prioritization.",                effects: { scope: 1 },          consequence: "The competitor changes your roadmap more effectively than your customers do." },
      { label: "Commit it to V2 with a written rationale", desc: "Buy time using paragraphs.",                              effects: { scope: 1 },               consequence: "The board receives an answer. The roadmap survives another quarter." },
    ],
  },
  {
    id: 'STRAT-012',
    ...
    choices: [
      { label: "Acknowledge it and cut back",             desc: "Trim the roadmap and quietly pretend it was the plan all along.", effects: { scope: 1 }, consequence: "Three features disappear. Nobody remembers who asked for them. Progress accelerates." },
      { label: "Keep it. It's all valuable.",             desc: "Explain that each addition was strategically important.",          effects: { scope: -1 },                        consequence: "The project enters a new phase known as 'just one more thing.'" },
      { label: "Document it and make it official",        desc: "Put the scope creep in a PowerPoint and see what happens.",       effects: { trust: 1 },                          consequence: "The project is no longer off track. The track has been redrawn." },
      { label: "Blame the stakeholders who requested it", desc: "An exciting new approach to relationship management.",            effects: { trust: -1 },            consequence: "The room suddenly becomes very interested in who requested what." },
    ],
  },
  {
    id: 'STRAT-013',
    ...
    choices: [
      { label: "Call a meeting and decide together",  desc: "Surely one more meeting will fix this.",               effects: { trust: 1 },    consequence: "Alignment is achieved shortly after everyone gets tired." },
      { label: "Pick activation rate",                desc: "A dangerous amount of faith in product thinking.",      effects: { quality: 1 }, consequence: "Product wins. Marketing creates a new deck and calendar invite." },
      { label: "Let each team track their own metric", desc: "Three dashboards. Three realities. One product.",      effects: { quality: -1 },           consequence: "A VP asks for the north star metric. Three people answer simultaneously." },
      { label: "Create a North Star Task Force",       desc: "Nothing says urgency like a committee.",               effects: { morale: -1 },   consequence: "The problem disappears from your calendar and reappears in performance reviews." },
    ],
  },
  {
    id: 'STRAT-014',
    ...
    choices: [
      { label: "Reset the timeline",                desc: "Tell the truth while it's still affordable.",                     effects: { trust: -1 },   consequence: "Expensive. Honest. The team is relieved you didn't try to force it." },
      { label: "Find a workaround together",        desc: "Convert a blocker into a creative writing exercise.",             effects: { tech: 1 },   consequence: "The workaround becomes a permanent architecture decision." },
      { label: "Cut the feature from MVP",          desc: "The MVP yearns to be smaller.",                                   effects: { scope: 1 }, consequence: "You removed one feature and gained three executive conversations." },
      { label: "Ask why this wasn't raised sooner", desc: "A reasonable question with terrible timing.",                    effects: { morale: -1 },            consequence: "The postmortem starts before the incident ends." },
    ],
  },
  {
    id: 'STRAT-015',
    ...
    choices: [
      { label: "Push back on the initiatives",                  desc: "Treat scope as a finite resource.",                              effects: { scope: 1 },    consequence: "Necessary. Uncomfortable. The VP will find another angle." },
      { label: "Accept them and adjust the OKRs",               desc: "Rewrite the roadmap and hope the math works out later.",         effects: { scope: -1 },               consequence: "The roadmap now contains enough work for several timelines." },
      { label: "Ask the VP to prioritize them",                 desc: "A surprisingly controversial request.",                          effects: { trust: 1 },                 consequence: "The VP responds, 'They're all important.'" },
      { label: "Submit vague OKRs that accommodate everything", desc: "Improve platform outcomes through strategic initiatives.",       effects: { quality: -1 }, consequence: "The OKRs survive review because nobody can disagree with them." },
    ],
  },
  {
    id: 'STRAT-016',
    ...
    choices: [
      { label: "Add their feature",                        desc: "Make the roadmap everyone's problem.",                      effects: { scope: -1 },               consequence: "The stakeholder is delighted. Somewhere else in the company, a PM experiences a sudden unexplained sense of loss." },
      { label: "Explain the prioritization",               desc: "Introduce logic into an emotional conversation.",           effects: { trust: 1 }, consequence: "The stakeholder agrees the process is fair and remains completely unsatisfied." },
      { label: "Say it's coming in a later phase",         desc: "The safest place for a feature request is the future.",    effects: { trust: -1 },   consequence: "Future-you inherits a commitment present-you never intended to make." },
      { label: "Ask what problem they're trying to solve", desc: "A dangerous amount of curiosity.",                         effects: { quality: 1 },            consequence: "The requested feature disappears. Three new problems appear." },
    ],
  },
],
  2: [ // Design
  {
    ...
    choices: [
      { label: "Prioritize clarity over visual purity",        desc: "A dashboard is not a perfume ad.",                       effects: { quality: 1 },        consequence: "The design loses some of its minimalist mystique and gains the radical ability to be useful." },
      { label: "Ship the clean version and train users",       desc: "If they cannot find it, perhaps they did not deserve it.", effects: { quality: -1 },    consequence: "Support tickets become the product's most successful engagement channel." },
      { label: "Create a compromise with progressive disclosure", desc: "Keep the surface clean, but let serious users dig deeper.", effects: { quality: 1 }, consequence: "The advanced controls move one click deeper instead of three." },
      { label: "Ask design to complete real tasks using the mockup", desc: "A gentle invitation to suffer like the customer.", effects: { quality: 1 }, consequence: "The phrase 'where did we put that?' enters the design review." },
    ],
  },
  {
    id: 'DES-018',
    ...
    choices: [
      { label: "Cut the flourishes and keep the core flow",    desc: "Preserve the value. Sacrifice the interpretive dance.",   effects: { scope: 1 }, consequence: "The experience gets simpler, the build gets saner, and the animation is quietly escorted out of the building." },
      { label: "Approve the full design vision",               desc: "Commit to the deluxe edition of the feature.",           effects: { scope: -1 }, consequence: "Several developers develop a personal relationship with technical debt." },
      { label: "Build the basic version now, polish later",    desc: "Launch the skeleton. Add the cheekbones in a future sprint.", effects: { tech: 1 }, consequence: "You preserve momentum, though the final product temporarily has the charisma of a tax portal." },
      { label: "Ask design to rank what actually matters most", desc: "Turn taste into tradeoffs.",                            effects: { trust: 1 },             consequence: "Several pixels are asked to justify their existence." },
    ],
  },
  {
    id: 'DES-019',
    ...
    choices: [
      { label: "Redesign now",                          desc: "Fix it properly while you still can.",        effects: { quality: 1 },            consequence: "The designer stops making that face." },
      { label: "Ship it and iterate post-launch",       desc: "Let users discover the problem for you.",    effects: { quality: -1 },         consequence: "You collect a statistically significant amount of disappointment." },
      { label: "Quick fix that partially addresses it", desc: "Make it less bad. Ship it. Repeat.",         effects: { quality: 1 },                      consequence: "Failure drops from 100% to a more respectable number." },
      { label: "Dismiss the finding",                   desc: "One data point. Probably an outlier. Almost certainly not.", effects: { quality: -1 }, consequence: "'One tester's opinion.' The finding shows up in your App Store reviews three weeks later." },
    ],
  },
  {
    id: 'DES-020',
    ...
    choices: [
      { label: "Side with UX. Usability first.",          desc: "Remind everyone that users still need to find the buttons.",              effects: { quality: 1 }, consequence: "Users will thank you. Brand sends a passive-aggressive Slack message." },
      { label: "Side with Brand. It needs to stand out.", desc: "Boldness is part of the experience.",                                    effects: { trust: 1 }, consequence: "It looks great. Users will figure it out... eventually." },
      { label: "Commission a usability test to settle it", desc: "Introduce data into the argument and watch both sides reinterpret it.", effects: { scope: -1 }, consequence: "The creative tension period is replaced by a findings interpretation period." },
      { label: "Try a compromise design",                  desc: "Produce something that disappoints everyone equally.",                  effects: { morale: -1 }, consequence: "Two weeks of debate successfully produce one shade of blue." },
    ],
  },
  {
    id: 'DES-021',
    ...
    choices: [
      { label: "Fix all 12. Ship it right.",       desc: "Do the thing everyone should do and almost nobody does.", effects: { quality: 1 },   consequence: "Three weeks disappear. So do twelve accessibility issues." },
      { label: "Fix the critical 4, log the rest", desc: "Triage and document. The 8 will come back.",             effects: { quality: 1 },             consequence: "The highest-risk problems disappear. The others secure long-term housing in the backlog." },
      { label: "Ship and fix in the next sprint",  desc: "Make a promise you might not keep.",                     effects: { quality: -1 }, consequence: "Everyone agrees the issues should be fixed eventually. Eventually remains flexible." },
      { label: "Deprioritize. Not in the OKRs.",   desc: "It didn't make the list. Neither will your credibility.", effects: { quality: -1 }, consequence: "The designer mutes their microphone before responding." },
    ],
  },
  {
    id: 'DES-022',
    ...
    choices: [
      { label: "Let the designer prototype all three",    desc: "Let the ideas fight.",                                  effects: { scope: -1 },            consequence: "Three ideas enter. One leaves. The process works." },
      { label: "Have engineering build the first option", desc: "Move fast and become attached to it.",                 effects: { quality: -1 }, consequence: "The feature ships. The designer develops a thousand-yard stare." },
      { label: "Prototype on paper. No code yet.",        desc: "A surprisingly offensive amount of practicality.",      effects: { quality: 1 }, consequence: "Everyone learns something. Nobody gets attached to pixels." },
      { label: "Put it to a team vote",                   desc: "Destroy bad ideas before they acquire code.",           effects: { morale: 1 }, consequence: "You discover that preference and performance are different metrics." },
    ],
  },
  {
    id: 'DES-023',
    ...
    choices: [
      { label: "Allow the full rebuild",                desc: "Choose violence against technical debt.",          effects: { scope: -1 },   consequence: "You spend six months paying off debt instead of refinancing it." },
      { label: "Decline. Patch the existing library.",  desc: "Keep the patient comfortable.",                   effects: { tech: -1 }, consequence: "Button_Primary_Final becomes Button_Primary_Final_v2." },
      { label: "Refactor only the critical components", desc: "Treat the symptoms.",                             effects: { tech: 1 },    consequence: "The biggest pain disappears. The weirdness remains." },
      { label: "Bring in a contractor to assess",       desc: "Buy a second opinion.",                           effects: { trust: 1 },     consequence: "The rebuild gains legitimacy through the ancient ritual of being expensive." },
    ],
  },
  {
    id: 'DES-024',
    ...
    choices: [
      { label: "Translate the feedback into specific UX issues", desc: "Take the scenic route to clarity.",        effects: { quality: 1 },          consequence: "The team rediscovers the power of concrete nouns." },
      { label: "Side with design and redo the layout",           desc: "The mockup has spoken.",                  effects: { scope: -1 }, consequence: "The design gets cleaner, but the team loses a chunk of time to a problem nobody could originally define." },
      { label: "Side with engineering and keep it moving",       desc: "The button can be clicked. What more do you want?", effects: { trust: -1 }, consequence: "Progress continues, and design begins documenting your future mistakes with unusual precision." },
      { label: "Put the design in front of users before changing it", desc: "Let civilians decide.",              effects: { quality: 1 }, consequence: "Users identify a completely different problem." },
    ],
  },
],
  3: [ // Development
  {
    ...
    choices: [
      { label: "Add it to this sprint",               desc: "The sprint yearns to grow.",                        effects: { scope: -1 }, consequence: "The stakeholder remains convinced it was small. Three engineers quietly update their resumes." },
      { label: "Push it to next sprint",              desc: "Future-you has availability.",                      effects: { scope: 1 }, consequence: "The sprint survives intact. The stakeholder acts disappointed in the ceremonial way." },
      { label: "Trade it for something of equal size", desc: "Conservation of suffering.",                       effects: { scope: 1 }, consequence: "The request remains urgent right up until it costs something." },
      { label: "Ask for a written rationale and impact", desc: "Introduce paperwork.",                           effects: { trust: 1 }, consequence: "Additional context is requested. The request is never mentioned again." },
    ],
  },
  {
    ...
    choices: [
      { label: "Allow it.",                         desc: "Pay the maintenance bill voluntarily.",                    effects: { tech: 1 },   consequence: "The refactor prevents several future disasters and creates no PowerPoint slides." },
      { label: "Decline and scope it for next quarter", desc: "Kick the can with executive precision.",              effects: { tech: -1 }, consequence: "The debt compounds at a competitive interest rate." },
      { label: "Allow a 3-day timebox",             desc: "Place boundaries around hope.",                            effects: { tech: 1 },   consequence: "Three days becomes four. Everyone pretends not to notice." },
      { label: "Reframe it as a feature request",   desc: "If it has a name, it can have a roadmap slot.",            effects: { trust: -1 }, consequence: "You solved a prioritization problem by renaming it." },
    ],
  },
  {
    ...
    choices: [
      { label: "Escalate through management",            desc: "Convert a dependency into a political event.",        effects: { trust: -1 }, consequence: "It works. The platform lead responds within the hour. You now have an enemy on the 4th floor." },
      { label: "Build a temporary workaround",           desc: "Nothing is more permanent than temporary.",           effects: { tech: -1 }, consequence: "Future engineers will assume it exists for a reason." },
      { label: "Reorder the sprint. Build around it.",   desc: "Practice agile in its natural habitat.",              effects: { morale: 1 },           consequence: "Nothing moves forward exactly as planned. Everything moves forward enough." },
      { label: "Document it and formally flag the risk", desc: "Escalate the issue to Confluence.",                   effects: { trust: 1 },            consequence: "The problem remains unsolved but exceptionally well documented." },
    ],
  },
  {
    ...
    choices: [
      { label: "Address all three in sequence",        desc: "Fix everything. Ship nothing on time.",                 effects: { scope: -1 },  consequence: "The infrastructure improves dramatically. Product development becomes a seasonal activity." },
      { label: "Triage with the team. Pick one.",      desc: "Make a call together. Log the other two.",             effects: { tech: 1 },  consequence: "Good process. One concern was real. Two were vibes. Nobody says this." },
      { label: "Push all three to post-launch",        desc: "Ship now. Pay later. With interest.",                  effects: { tech: -1 }, consequence: "You shipped faster. The three engineers now have a group chat you're not in." },
      { label: "Bring in a senior engineer to assess", desc: "Get outside credibility to end the internal debate.",  effects: { tech: 1 },   consequence: "The debate ends when someone everyone respects says the same thing." },
    ],
  },
  {
    ...
    choices: [
      { label: "Mandate docs in the sprint process",  desc: "Make forgetting more difficult.",                        effects: { tech: 1 },  consequence: "The code becomes self-documenting in the same way cats are self-walking." },
      { label: "Hire a technical writer",             desc: "Summon the chosen one.",                                 effects: { tech: 1 },    consequence: "Greg, the technical writer becomes the most popular person on the team." },
      { label: "Let it go until post-launch",         desc: "Tomorrow's problem deserves tomorrow's attention.",       effects: { tech: -1 }, consequence: "Institutional knowledge continues its proud tradition of being institutional and not knowledge." },
      { label: "Make documentation a launch blocker", desc: "Weaponize process.",                                     effects: { morale: -1 },  consequence: "Critical business logic is preserved in Slack messages from 2024." },
    ],
  },
  {
    ...
    choices: [
      { label: "Add it.",                   desc: "The scope hungers.",                                          effects: { scope: -1 },           consequence: "The launch checklist gains a new item. The team gains a new eye twitch." },
      { label: "Decline.",                  desc: "Scope is a finite resource. Allegedly.",                      effects: { scope: 1 },         consequence: "The feature is rejected. The PowerPoint advocating for it is quietly becoming longer." },
      { label: "Add a simplified version.", desc: "Build the smallest thing everyone can still complain about.", effects: { scope: -1 },         consequence: "Leadership immediately renames the simplified version to Phase 1." },
      { label: "Ask for the business impact.", desc: "Require the request to survive contact with numbers.",     effects: { trust: 1 }, consequence: "Someone says 'strategic value.' Nobody can define it." },
    ],
  },
  {
    ...
    choices: [
      { label: "Cut scope and finish the core flow",          desc: "Save the feature from itself.",                    effects: { scope: 1 }, consequence: "The feature gets smaller, more coherent, and much less likely to humiliate you in demo." },
      { label: "Push the team to finish everything",          desc: "Reality is a mindset.",                            effects: { morale: -1 }, consequence: "The team delivers everything. Quality Assurance files a missing persons report." },
      { label: "Move the unfinished work to next sprint",     desc: "Delay with professional vocabulary.",              effects: { scope: 1 }, consequence: "Several tickets achieve seniority." },
      { label: "Ask for a live walkthrough of what actually works", desc: "The demo cannot hurt you if you hurt it first.", effects: { quality: 1 }, consequence: "Three blockers, two assumptions, and one architectural crime are discovered." },
    ],
  },
  {
    ...
    choices: [
      { label: "Let design decide.",          desc: "The adults are handling it.",         effects: { quality: 1 },              consequence: "Design chooses a slightly different shade of orange. The conversion rate remains blissfully unaware." },
      { label: "End the debate and move on.", desc: "Declare victory and continue.",       effects: { morale: 1 },  consequence: "The debate ends. The color stays. Someone will bring this up in retro." },
      { label: "Escalate the decision.",      desc: "The button deserves governance.",     effects: { morale: -1 },    consequence: "The button receives executive sponsorship." },
      { label: "Run an A/B test.",            desc: "The scientific method enters the chat.", effects: { scope: -1 }, consequence: "Months later, the button is still orange." },
    ],
  },
  {
    ...
    choices: [
      { label: "Trust the estimate.",          desc: "What are the odds it happens twice?",                     effects: { trust: 1 },       consequence: "The roadmap quietly updates itself to local time." },
      { label: "Ask for a simpler version.",   desc: "Negotiate with the laws of software development.",        effects: { scope: 1 },  consequence: "Simpler is defined in the meeting. The simpler version also takes two weeks." },
      { label: "Ask what happened last time.", desc: "Compare the estimate to its natural habitat.",            effects: { trust: -1 },    consequence: "The team discovers 'two weeks' is a unit of optimism, not time." },
      { label: "Double the estimate privately.", desc: "Apply the industry standard conversion rate.",          effects: { trust: -1 }, consequence: "Engineering estimates in dog years." },
    ],
  },
  {
    ...
    choices: [
      { label: "Break work into smaller deliverables today",         desc: "Reduce ambiguity until progress has fewer places to hide.",    effects: { quality: 1 },   consequence: "Several tickets are discovered to be ecosystems." },
      { label: "Push for longer hours to catch up",                  desc: "Convert bad planning into human fatigue.",                    effects: { morale: -1 }, consequence: "The team works nights. The bugs work weekends." },
      { label: "Drop lower-priority tickets now",                    desc: "Admit reality before reality schedules the meeting for you.", effects: { scope: 1 },   consequence: "Reality arrives carrying scissors. Everyone agrees the cuts make sense and privately believes their ticket should have survived." },
      { label: "Ask each engineer to show what changed since yesterday", desc: "Replace progress poetry with receipts.",                 effects: { trust: 1 }, consequence: "Status updates evolve into evidence." },
    ],
  },
],
  4: [ // Testing
  {
    ...
    choices: [
      { label: "Delay launch to fix it.",           desc: "Protect users from your calendar.",              effects: { quality: 1 },           consequence: "Stakeholders are upset for a week. Users remain blissfully unaware forever." },
      { label: "Launch and hotfix within 24 hours", desc: "The bug can enjoy a brief production tour.",     effects: { quality: -1 },          consequence: "The hotfix becomes the real launch." },
      { label: "Remove the affected feature.",      desc: "Save the product from one of its features.",    effects: { scope: 1 },                     consequence: "The feature is escorted out of the release with dignity." },
      { label: "Ship it.",                          desc: "A bold commitment to learning.",                 effects: { quality: -1 }, consequence: "The bug was successfully promoted to production." },
    ],
  },
  {
    ...
    choices: [
      { label: "Tag and categorize everything",             desc: "Turn user pain into color-coded tabs.",             effects: { morale: -1 }, consequence: "Nothing has been fixed yet, but the spreadsheet is magnificent." },
      { label: "Find the top 10 by frequency and fix them", desc: "Let the complaints compete for resources.",         effects: { quality: 1 },             consequence: "The phrase 'more like Excel' becomes a recurring character." },
      { label: "Hand it to the team to self-organize",      desc: "Throw 200 opinions into a room and lock the door.", effects: { quality: -1 }, consequence: "The signal emerges slowly from the noise, like a submarine surfacing." },
      { label: "Present all 200 items to stakeholders",     desc: "Share every thought a user has ever had.",          effects: { morale: -1 }, consequence: "Three hours later, the group has strong opinions about item #183." },
    ],
  },
  {
    ...
    choices: [
      { label: "Delay launch until it's resolved.",       desc: "The infrastructure has requested diplomatic talks.",            effects: { quality: 1 },          consequence: "A future postmortem quietly fails to occur." },
      { label: "Launch with soft rate limiting.",         desc: "First come, first served. Everyone else gets character development.", effects: { quality: -1 }, consequence: "You successfully solve a scaling problem by preventing scale." },
      { label: "Invest in performance optimization now.", desc: "The code would like to discuss its lifestyle choices.",        effects: { tech: 1 },             consequence: "The optimization effort uncovers several crimes against software." },
      { label: "Launch to a waitlist to control load.",   desc: "Turn server limitations into brand mystique.",                 effects: { quality: 1 },          consequence: "The app can support 500 users comfortably and 2,000 users spiritually." },
    ],
  },
  {
    ...
    choices: [
      { label: "Push the release date by a week.",          desc: "Give the bugs more time to introduce themselves.",       effects: { tech: 1 },                consequence: "Three critical bugs are discovered. Ethan already knew about all of them." },
      { label: "Launch anyway.",                            desc: "Let production answer the questions nobody else can.",   effects: { quality: -1 }, consequence: "At 11:43 p.m., Ethan achieves legendary status from a beach." },
      { label: "Strip out the risky bits and ship the rest.", desc: "A classic move for anyone who enjoys survival.",      effects: { scope: 1 },                        consequence: "The release becomes smaller, uglier, and much less likely to ruin your weekend." },
      { label: "Ask who is covering for Ethan.",            desc: "Nothing clarifies launch risk like naming the backup plan.", effects: { trust: 1 },            consequence: "For the first time, the company attempts to determine exactly what Ethan does all day." },
    ],
  },
  {
    ...
    choices: [
      { label: "Block launch until staging is fixed.",      desc: "Stop pretending the dollhouse is the real house.",          effects: { tech: 1 },       consequence: "You spend a week fixing staging and accidentally improve engineering culture." },
      { label: "Ship anyway and monitor prod.",             desc: "Let reality handle the final QA pass.",                    effects: { quality: -1 }, consequence: "The monitoring dashboard achieves sentience." },
      { label: "Manually test only the riskiest flows.",   desc: "Check the parts most likely to ruin everyone's evening.", effects: { quality: 1 },     consequence: "Production continues its role as the company's most realistic testing environment." },
      { label: "Ask engineering how staging drift got this bad.", desc: "Turn discomfort into a process conversation.",      effects: { trust: -1 }, consequence: "Several temporary decisions are discovered to be celebrating birthdays." },
    ],
  },
  {
    ...
    choices: [
      { label: "Delay the demo.",             desc: "Buy time. Find the real problem.",         effects: { trust: -1 },          consequence: "The demo moves. The panic stays exactly where it was." },
      { label: "Proceed confidently anyway.", desc: "Confidence is half the demo.",             effects: { tech: -1 }, consequence: "The demo works for the first three clicks, which briefly feels like victory." },
      ...
      { label: "Use screenshots and a backup recording.", desc: "Demo the dream, not the implementation.", effects: { trust: 1 }, consequence: "Nobody notices until someone asks to click something." },
    ],
  },
  {
    ...
    choices: [
      { label: "Push the release a week.",                          desc: "Choose shame now over chaos later.",                               effects: { tech: 1 }, consequence: "The product is ready for launch in the same way a raccoon is ready for a wedding." },
      { label: "Launch anyway.",                                    desc: "Let production answer the questions nobody else can.",              effects: { quality: -1 }, consequence: "Several assumptions complete their final journey." },
      { label: "Strip out the risky bits and ship the rest.",       desc: "A classic move for anyone who enjoys survival.",                   effects: { scope: 1 }, consequence: "The release approaches with the confidence of a student who has not opened the textbook." },
      { label: "Make the loudest person own the launch decision.",  desc: "A bold governance model based entirely on consequences.",          effects: { trust: 1 },           consequence: "Risk tolerance falls sharply once someone suggests being on call." },
    ],
  },
  {
    ...
    choices: [
      { label: "Triage every bug and rank by user pain", desc: "Replace category theater with actual prioritization.",           effects: { quality: 1 }, consequence: "Several bugs are downgraded from 'critical' to 'annoying.' One moves in the opposite direction." },
      { label: "Close the edge cases and ship.",         desc: "Trust that users will stay politely on the happy path.",        effects: { quality: -1 }, consequence: "The release goes smoothly for the fictional user you optimized for." },
      { label: "Fix only the bugs tied to core flows.",  desc: "Protect login, checkout, save, and anything else capable of public humiliation.", effects: { quality: 1 }, consequence: "One bug survives every triage session through sheer charisma." },
      { label: "Make bug owners demo the issue live.",   desc: "A miracle cure for loose opinions.",                            effects: { trust: 1 },             consequence: "Half the arguments vanish once people have to reproduce their confidence in front of witnesses." },
    ],
  },
],
}
